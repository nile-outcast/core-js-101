...
function getPolynom(...args) {
  return function Func(x) {
    ...
    const a = length === 3 ? args[0] : 0;
    const b = length > 1 ? args[length - 2] : 0;
...
function logger(func, logFunc) {
  ...
  return function Func(...args) {
    log = `${func.name}(${args.map((item) => JSON.stringify(item))}) starts`;
    logFunc(log);
    const result = func(...args);
    log = `${func.name}(${args.map((item) => JSON.stringify(item))}) ends`;
    logFunc(log);
    return result;
  };
}
