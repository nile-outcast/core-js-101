...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1X < rect2.left || rect2X < rect1.left) return false;
  if (rect1Y < rect2.top || rect2Y < rect1.top) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  const diffX = point.x - circle.center.x;
  const diffY = point.y - circle.center.y;
  return diffX ** 2 + diffY ** 2 < circle.radius ** 2;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (start.indexOf(str[i]) === -1 && end.indexOf(str[i]) === -1) {
      idx = i;
      break;
    }
  }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const getStartBasket = (bool) => {
    if (bool) return '[';
    return '(';
  };
  const getEndBasket = (bool) => {
    if (bool) return ']';
    return ')';
  };
  const startStr = a < b ? `${getStartBasket(isStartIncluded)}${a}` : `${getStartBasket(isEndIncluded)}${b}`;
  const endStr = a < b ? `${b}${getEndBasket(isEndIncluded)}` : `${a}${getEndBasket(isStartIncluded)}`;
