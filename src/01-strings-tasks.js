...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, startInd) + str.slice(startInd + lengthVal);
}
...
function getRectangleString(width, height) {
  ...
  const firstString = `┌${dash}┐\n`;
  const lastString = `└${dash}┘\n`;
  const middleString = `│${space}│\n`;
...
function getCardId(value) {
  ...
  const indexRanks = ranks.indexOf(value.slice(0, -1));
  const numSuits = suits[value.slice(-1)];

  return indexRanks + numSuits;
}
